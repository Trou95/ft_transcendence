...
export class MatchService
{
    ...
    async getTotalWins() {
        ...
        const scores = new Map<number, { full_name?: string, avatar?: string, score: number }>();

        matchs.forEach((m: any) => {
            const players: number[] = [m.player1.id, m.player2.id];

            if (!scores.get(players[0]))
                scores.set(players[0], { full_name: m.player1.full_name, avatar: m.player1.avatar, score: 0 });
            if (!scores.get(players[1]))
                scores.set(players[1], { full_name: m.player2.full_name, avatar: m.player2.avatar, score: 0 });

            if (m.score1 > m.score2)
                scores.set(players[0], { ...scores.get(players[0]), score: scores.get(players[0]).score + 1 });
            else if (m.score2 > m.score1)
                scores.set(players[1], { ...scores.get(players[1]), score: scores.get(players[1]).score + 1 });
        });
